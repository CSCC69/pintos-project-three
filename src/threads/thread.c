...
add_fd_file (struct thread *t, struct file *f)
{
  ...
  if (list_empty (&t->fd_file_closed))
    ...
    {
      fd_file = list_entry (list_pop_front (&t->fd_file_closed),
                            struct fd_file, list_elem);
      fd_file->file = f;
    }
...
remove_fd_file (struct thread *t, int fd)
{
  ...
  if (fd == fd_file->fd)
    ...
    list_insert_ordered (&t->fd_file_closed, &fd_file->list_elem,
                         fd_closed_less, NULL);
...
struct file *
get_open_file (struct thread *t, int fd)
{
  struct fd_file *fd_file = get_fd_file (t, fd);
  if (fd_file == NULL)
    return NULL;
...
free_fds (struct thread *t)
{
  ...
  struct list_elem *e;
  for (e = list_begin (&t->fd_file_closed); e != list_end (&t->fd_file_closed);
       e = list_next (e))
    {
...
/* Frees the file struct assoicated with a fd_file struct, and then frees the
 * fd_file in a hash map */
void
...
/* Computes the hash of a file descriptor for lookup & insertion of fd_file
 * structs in a hash map */
unsigned
...
{
  const struct spt_entry *spt_entry
      = hash_entry (spt_entry_, struct spt_entry, elem);
  return hash_int ((uint32_t)spt_entry->upage);
}

bool
spt_entry_less (const struct hash_elem *spt_entry_1,
                const struct hash_elem *spt_entry_2, void *aux UNUSED)
{
  return hash_entry (spt_entry_1, struct spt_entry, elem)->upage
         < hash_entry (spt_entry_2, struct spt_entry, elem)->upage;
}
